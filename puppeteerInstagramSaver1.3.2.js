...
(async () => {
    console.log('puppeteer Instagram Saver1.3.2');
    console.log('Author: Nakateru(2022.1.21)');
...
/*    await page.waitForSelector('#loginForm', {visible: true});
    ...
    function parseJson(content) {
        ...
        var typeCount = res.items[0].carousel_media_count;

        if (typeCount !== undefined) {
            //image(s) and video(s)
            console.log('Found',typeCount,'photo(s) or video(s) in this post');
            var arr = []
            res.items[0].carousel_media.forEach(i => {
                if(i.media_type === 1){
                    arr.push(i.image_versions2.candidates[0].url);
                }else{
                    arr.push(i.video_versions[0].url);
                }

            });
            return arr;
            //only 1 image [return string]
        } else if (res.items[0].media_type === 1) {
            console.log('Found 1 photo in this post');
            return res.items[0].image_versions2.candidates[0].url;
            //only 1 video [return string]
        } else {
            console.log('Found 1 video in this post');
            return res.items[0].video_versions[0].url;
        }
    }
